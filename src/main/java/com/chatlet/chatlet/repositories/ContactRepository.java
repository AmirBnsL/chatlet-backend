...
public interface ContactRepository extends JpaRepository<Contact, Long>{
    ...
    @Query(value = "SELECT c.id,username FROM contact c join auth a on a.id = c.user_id OR a.id=c.contact_id join profile p on a.id = p.id WHERE (contact_id=:idUser OR user_id=:idUser) AND (a.id!=:idUser)",nativeQuery = true)
    Collection<ContactInfo> findAllContactsInfoByUserId(Long idUser);
