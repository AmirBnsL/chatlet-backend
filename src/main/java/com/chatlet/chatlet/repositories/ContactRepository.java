...
public interface ContactRepository extends JpaRepository<Contact, Long>{
    ...
    @Query(value = "SELECT picture_link,username FROM contact c join auth a on a.id = c.user_id OR a.id=c.contact_id join profile p on a.id = p.id WHERE (contact_id=:id OR user_id=:id) AND (a.id!=:id)",nativeQuery = true)
